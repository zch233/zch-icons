...
onMounted(() => {
    ...
    function addEyeCircleProperty(circleData, eye, cachedMouseEyeData) {
        //
        var eyeContainer = animationAPI.getKeyPath(eye + ',Contents,' + circleData.name + ',Transform,Position');
        var lastValue = null,
            eye_angle;
        animationAPI.addValueCallback(eyeContainer, function (currentValue) {
            ...
            if (!isActive) {
                ...
                trasformedPoint = animationAPI.toKeypathLayerPoint(eyeContainer, trasformedPoint);
                cachedMouseEyeData.distance = Math.sqrt(Math.pow(trasformedPoint[0], 2) + Math.pow(trasformedPoint[1], 2));
                cachedMouseEyeData.eye_angle = Math.atan2(0 - trasformedPoint[1], 0 - trasformedPoint[0]) / degToRads + 179;
            ...
            var newValueX = currentValue[0] + distance * Math.cos(eye_angle * degToRads);
            var newValueY = currentValue[1] + distance * Math.sin(eye_angle * degToRads);
            lastValue[0] = lastValue[0] + ((newValueX - lastValue[0]) / circleData.divisor) * 3;
            lastValue[1] = lastValue[1] + ((newValueY - lastValue[1]) / circleData.divisor) * 3;
            //return currentValue;
            return lastValue;
        });
    }
    ...
    function addEyeCircles() {
        ...
        for (i = 0; i < len; i += 1) {
            addEyeCircleProperty(leftEyeCircles[i], 'Loop,left_eye', cachedMouseEyeData);
        }
        ...
        // len = 1;
        for (i = 0; i < len; i += 1) {
            addEyeCircleProperty(rightEyeCircles[i], 'Loop,right_eye', cachedMouseEyeData);
        }
    }

    function changeColor(leave_name) {
        var leafColorKey = animationAPI.getKeyPath('#' + leave_name + ',Contents,color_group,fill_prop,Color');
    ...
    function addMouthProperties() {
        ...
        var keyPathMouthContainerTimeRemap = animationAPI.getKeyPath('Mouth,Time Remap');
        var perc = 0;
        animationAPI.addValueCallback(keyPathMouthContainerTimeRemap, function (currentValue) {
            if (!isActive && mouse_changed) {
                ...
                distanceToMouse = Math.sqrt(Math.pow(0 - point2[0], 2) + Math.pow(0 - point2[1], 2));
                mouse_changed = false;
            }
            ...
            if (distanceToMouse < minTongueRadius) {
                ...
                perc = 1 - Math.min(1, (distanceToMouse - maxTongueRadius) / (maxTongueRadius + 100));
                return perc * (9 / 30);
    ...
    function addTongueProperties() {
        ...
        var keyPathTongueContainerTimeRemap = animationAPI.getKeyPath('Mouth,Tongue_Comp,Time Remap');
        animationAPI.addValueCallback(keyPathTongueContainerTimeRemap, function (currentValue) {
            if (distanceToMouse > minTongueRadius && distanceToMouse < maxTongueRadius && !isActive) {
                animateTongue();
            }
            if (isActive) {
        ...
        var keyPathTongueContainer = animationAPI.getKeyPath('Mouth,Tongue_Comp,Transform,Rotation');
...
.xxx {
    .theLottie {
        ...
        & #mouse-container.active {
            cursor: url(https://labs.nearpod.com/bodymovin/demo/chameleon/cursor_empty_32.png),
                url(https://labs.nearpod.com/bodymovin/demo/chameleon/cursor_empty_32.cur), auto;
        }
